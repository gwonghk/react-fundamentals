...
1. Imperative(HOW)(BrowserAPI) vs Declarative(WHAT)(React)
  Imperative is giving step by step instructions
  Declarative is saying what you want
  Imperative coding is going through a forloop to manipulate array
  Declarative is to use the .map or .forEach or .reduce functions
  Declarative is easier to transplant into different coding languages
  Functional programming is a subset of declarative programming.

2. React & ReactDOM
   React does the document.createElement() bit
   ReactDOM does the rootElement.append() bit

3. React.createElement(type, \[props\], \[...children\])
   Third argument, children, can be just string like this
   React.createElement('span', null, 'Hello').
   Subsequent arguments are just more children added inside.
